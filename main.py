...
sheet.cell(row=row, column=2).value = 'Квадратура'
sheet.cell(row=row, column=3).value = 'Этаж'
sheet.cell(row=row, column=4).value = 'Цена'
sheet.cell(row=row, column=5).value = 'Ссылка'
...
def getTitle(text):
    ...
    square = float((text[:text.find('м')-1:]).replace(',', '.'))
    text = text[text.find(',') + 2::]
    floor = text[:text.find('э')-1:]
    return square, floor
...
try:
    ...
    data = []
    while True:
        name = driver.find_elements(By.XPATH, constants.name)
        ...
        for i in range(len(pricelist)):
            (square, floor) = getTitle(name[i].text)
            data.append([adress[i].text, square, floor, pricelist[i].text, href[i].get_attribute('href')])
    ...
    for i in range(len(data)):
        ...
        sheet.cell(row=i+2, column=4).value = data[i][3]
        sheet.cell(row=i+2, column=5).value = data[i][4]
